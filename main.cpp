...
void snake::drawGround()
{
    ...
    for(int i = 0 ; i <= y_size-1 ; i++)
    {
        for(int j = 0 ; j <= x_size-1 ; j++)
        {
            ...
            {
                if (skin == 1) color(15);  //無色
                if (skin == 2) color(10);  //綠色
                if (skin == 3) color(9);   //藍色
                if (skin == 4) color(12);  //紅色
                cout << "&" ;
            }else if(ground[j][i] == OBSTACLE && ground[j][i] != WALL )
            {
                cout << "#" ;
    ...
    {
        newRandom:
        int new_x = rand() % (x_size) ; // x = 0~52
        int new_y = rand() % (y_size) ; // y = 0~27
        ...
        if(ground[new_x][new_y] == WALL)
        {
            ...
            if ((ground[0][new_y] == PORTAL || ground[52][new_y] == PORTAL) && ground[0][0] != PORTAL && ground[52][0] != PORTAL && ground[0][27] != PORTAL && ground[52][27] != PORTAL)
            {
                ...
                gotoXY(new_x, new_y);
                cout << "O";
                gotoXY(new_x, new_y + 1);
                cout << "O";
                gotoXY(new_x, new_y + 2);
                cout << "O";
            }
            ...
            else if ((ground[new_x][0] == PORTAL || ground[new_x][27] == PORTAL) && ground[0][0] != PORTAL && ground[52][0] != PORTAL && ground[0][27] != PORTAL && ground[52][27] != PORTAL)
            {
                ...
                gotoXY(new_x , new_y);
                cout << "O";
                gotoXY(new_x + 1 , new_y);
                cout << "O";
                gotoXY(new_x + 2 , new_y);
                cout << "O";
            }else
            {
                srand(rand());
...
int main()
{
    ...
    while(!gameOver)
    {
        ...
        //當分數 = 4 的倍數時 => 加速蛇的移動
        if(Snake.score % 4 == 0 && Snake.score != 0 && food_count == 4)
        {
